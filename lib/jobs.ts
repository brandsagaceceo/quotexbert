...
export async function saveInterest(params: {
  ...
  const { contractorId, leadId } = params;

  // For SAVED type, use SavedJob model
  ...
  // For APPLIED type, this would typically update the Lead model
  // or create a separate Application model, but for now we'll just return
  return {
    id: "applied",
    userId: contractorId,
    jobId: leadId,
...
export async function getContractorInterests(
  contractorId: string,
  type?: "SAVED" | "APPLIED",
) {
  ...
  // Since we're using SavedJob model, we only return saved jobs for now
  return await prisma.savedJob.findMany({
    ...
    orderBy: { createdAt: "desc" },
    include: {
      job: true, // Include the Lead details
    },
  });
}
